...
function setVocabularies(mode) {
	...
	for(let ii=0;ii<vocabularies.length;ii++) {
		sHtml += '<button onclick="clearInterval(hoge);eVideo.currentTime='+vocabularies[ii][TIME2_S].toString()+';eVideo.play();hoge = setInterval(function() {clearInterval(hoge);eVideo.pause();},'+((vocabularies[ii][TIME2_E]-vocabularies[ii][TIME2_S])*1000).toString()+');">'+vocabularies[ii][LANG]+'</button><br>';
	}
